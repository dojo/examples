...
describe('Custom Element Showcase - Dojo 2', function (this: any) {
		...
		it('should open when its open attribute is set to true, and close when set to false', () =>
			...
			// Check that is starts closed and verify the open attribute value
				...
				.then((visible: boolean) => assert.isFalse(visible,
					'Dialog should not be visible after the close button was clicked'))
				.then(() => page.getAttributeTruthiness(page.selectors.dialog, 'open'))
				.then((open: string) => assert.isFalse(open, 'Open attribute should be false after dialog is closed'))
		);
	});
	...
	describe('SlidePane', () => {
		...
		it('should become visible when open attribute is true, and close when it is false', () =>
			...
			// Verify it starts closed
				...
				.then(() => page.getAttributeTruthiness(page.selectors.slidePane, 'open'))
				.then((open: string) => assert.isTrue(open, 'Open attribute should be true when slide pane is open'))
				// Close the slide pane
				.then(() => page.click(page.selectors.slidePaneOverlay))
	...
	describe('AccordionPane', () => {
		it('should have both title panes closed by default', () =>
			page.getAttributeTruthiness(page.selectors.accordionTitlePaneOneContent, 'aria-hidden')
				.then((hidden: boolean) => assert.isTrue(hidden, 'Title pane content should not be visible be default'))
	...
	describe('Tooltip', () => {
		...
		it('should become visible based on its open attribute toggled', () =>
			...
			// Verify it starts hidden
				.then((visible: boolean) => assert.isFalse(visible, 'Tooltip should be hidden by default'))
				.then(() => page.getAttributeTruthiness(page.selectors.tooltip, 'open'))
				.then((open: string) => assert.isFalse(open, 'Open attribute should be false before tooltip is opened'))
				// Open the tooltip
				...
				.then(() => page.getAttributeTruthiness(page.selectors.tooltip, 'open'))
				.then((open: string) => assert.isTrue(open, 'Open attribute should be true when tooltip is open'))
				// Close the tooltip
				.then(() => page.click(page.selectors.tooltipButton))
				.then(() => page.isVisible(page.selectors.tooltipContent))
				.then((visible: boolean) => assert.isFalse(visible, 'Tooltip should not be visible after being closed'))
				.then(() => page.getAttributeTruthiness(page.selectors.tooltip, 'open'))
				.then((open: string) => assert.isFalse(open, 'Open attribute should be false after tooltip is closed'))
		);
	});
	...
	describe('calendar', () => {
		...
		it('should allow the day to be selected', () =>
			page.getAttributeTruthiness(page.selectors.calendarDay, 'aria-selected')
				.then((selected: boolean) => assert.isFalse(selected, 'Day should not be selected by default'))
				.then(() => page.click(page.selectors.calendarDay))
				.then(() => page.getAttributeTruthiness(page.selectors.calendarDay, 'aria-selected'))
				.then((selected: boolean) => assert.isTrue(selected, 'Day should be selected after clicking it'))
		);
	})
});
